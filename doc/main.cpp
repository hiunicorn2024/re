...
void doc_miscl() {
  ...
  // no_overflow_adding(integral_a, integral_b)->bool
  // add_with_overflow_check(integrals...)->optional<integral>
...
void doc_key_code() {
  ...
  // key_shift
  // key_control key_alt   key_space    key_up
...
void doc_key_combination() {
  // key_name(unsigned)->sview
  // key_code(sview)->optional<unsigned>
  // is_modifier_key(unsigned)->bool
  // is_upper_modifier_key(unsigned)->bool
  // is_lower_modifier_key(unsigned)->bool
  // to_upper_modifier_key(unsigned)->unsigned
  //
  // key_combination
  //   // if valid, back(*this) is the non-modifier key
  //   smf: full
  //   ==
  //   <=> // compare size for different sizes, otherwise compile sequence
  //   explicit key_combination(r)
  //   explicit key_combination(unsigned)
  //   key_combination(k1, k2)
  //   key_combination(k1, k2, k3)
  //   key_combination(k1, k2, k3, k4)
  //   valid()->bool
  //   explicit operator bool()
  ...
  //   size()
  //   empty()
  //   conflicts_with(const key_combination &)
  //   includes(const key_combination &)
...
void doc_key_mapper() {
  ...
  //
  //   rotate_wheel(int)
  //   has_wheel_function()->bool
  //   wheel_function(f)
  //   remove_wheel_function()->bool
  //
  //   clear()
  //   empty()
  //
  //   contains(pair<key_combination, key_action>)->bool
  //
  //   insert(pair<key_combination, key_action>,
  //          function<void ()>)->bool
  //   insert_or_assign(pair<key_combination, key_action>,
  //                    function<void ()>)->bool
  //   remove(pair<key_combination, key_action>)->bool
}
...
void doc_zrect2() {
  ...
  //   empty()->bool
  //
  //   includes_point(zvec2)->bool
  //
  //   intersection_with(const zrect2 &)->zrect2
  //   exclude_from_non_overlapping(circular_vector<zvec2> &)
}
void doc_pixmap() {
  // pixmap
  //   smf: full
  //   ==
  //
  //   explicit pixmap(zvec2)
  //   pixmap(zvec2, fll)
  //
  //   pixmap(zvec2, const pixmap &)
  //   pixmap(zvec2, const pixmap &, fll)
  //
  //   pixmap(zvec2, r)
  //
  //   width()->size_type
  ...
  //   sub_range(zrect2)
  //
  //   fill(zrect2, uint32_t)
  //
  //   ()(x, y)
  //   ()(zvec2)
  //   iter(x, y)
  //   iter(zvec2)
  //   includes_point(zvec2)
  //
  //   value_type = uint32_t
  //   reference
  //   const_reference
  ...
  //   resize(zvec2, fll)
  //
  //   cover(zvec2, const pixmap &)->zvec2
  //     // return replaced width and height
  //     // if out of range, do nothing
  //   cover(zvec2, const pixmap &, mix_f)->zvec2
  //     // return replaced width and height
  //     // if out of range, do nothing
  //     // mix_f eats two value of const uint32_t &
  //     // mix_f(old, new)
  //
  //   cover(zvec2, const pixmap &, zrect2)->zvec2
  //     // return replaced width and height
  //     // if out of range, do nothing
  //   cover(zvec2, const pixmap &, zrect2, mix_f)->zvec2
  //     // return replaced width and height
  //     // if out of range, do nothing
  //     // mix_f eats two value of const uint32_t &
  //     // mix_f(old, new)
...
void doc_freetype() {
  ...
  //   reference = value_type;
  //   const_reference = value_type;
  //   iterator = a_t::iterator;
  //   const_iterator = a_t::iterator;
  //   difference_type = a_t::difference_type;
  //   size_type = a_t::size_type;
  //   smf: full
  //   begin()
  //   end()
  //   size()->size_t
  ...
  //   empty()
  //   rectangle_size()->zvec2
  //   width()->size_t
  //   height()->size_t
  //   cover(pixmap &, zvec2 pos, uint32_t color)
  //
  // ft_face
  //   // thread-safe except constructor and destructor
  //   smf: only destructible
  //   ft_face(string file_data, height)
  //   size()->size_t
  //   index(u32_code)->unsigned
  //   index(sv)->unsigned // sv reprent a single character
  //   render(id)->ft_glyph
  //   render(id, u32_color)->pixmap
  //   [](unsigned long)->ft_glyph
  //   [](sview)->ft_glyph
  //
  // ft_cached_face
  //   // thread-safe except constructor and destructor
  //   smf: only destructible
  //   ft_cached_face(string file_data, height)
  //   size()->size_t // return the height passed to constructor
  //   [](unsigned long)->const ft_glyph &
  //   [](sview)->const ft_glyph &
  //
  // glyph
  //   smf: full
  //   value_type = unsigned char
  ...
  //   width()->size_t
  //   height()->size_t
  //   cover(bitmap &, zvec2 pos, uint32_t color)
  //   code()->uint32_t
  //   utf8_code()->string
  //
  // font
  //   smf: only destructible
  //   font(string font_data, height, cached = false)
  //   cached()->bool
  //   size()->size_t
  //   [](unsigned long)->glyph
  //   [](sview)->glyph
}
void doc_window_paint_traits() {
  // window_paint_traits
  //   uint32_t background_color
  //   uint32_t background_color_locked
  //   uint32_t forground_color
  //   uint32_t forground_color_shining
  //   uint32_t forground_color_locked
  //   uint32_t font_color
  //   uint32_t font_color_shining
  //   uint32_t font_color_locked
  //   uint32_t frame_color
  //   uint32_t frame_color_shinging
  //   uint32_t frame_color_locked
  //   uint32_t frame_color_warning
  //
  // default_window_paint_traits()->const window_paint_traits &
}
void doc_window() {
  // window
  // window_tree_vector = tree_vector<dynamic<window>>;
  // window_tree = tree<dynamic<window>>;
  // window_iterator = window_tree_vector::iterator;
  // window_const_iterator = window_tree_vector::const_iterator;
  //
  // deref_win(window_iterator)->window &
  // deref_win(window_const_iterator)->const window &
  // deref_win(window_iterator::value_type &)->window &
  // deref_win(const window_iterator::value_type &)->const window &
  // deref_win(x)->decltype(deref_win(*x))
  // deref_win(window *)->window &
  // deref_win(const window *)->const window &
  //
  // inner::fns::emplace_window(window_tree &, window_const_iterator, s...)
  // inner::fns::window_iter(window &, window_iterator);
  // inner::fns::window_size(window &, zvec2);
  // inner::fns::window_position(window &, zvec2);
  // inner::fns::window_paint(window &, pixmap &,
  //                          exclusive_thread_pool &, bool no_overlap)
  //
  // window // abstract class
  //   smf: only destructible
  //
  //   window(window_iterator)
  //
  //   tree_type = window_tree
  //   vector_type = window_tree_vector
  //   iterator = window_iterator
  //   const_iterator = window_const_iterator
  //   iter()->iterator/const_iterator
  //   protected iter(iterator)
  //
  //   protected rectangle_buffer_for_painting()->circular_vector<zrect2>_ref
  //     // only used by paint()
  //   protected absolute_rectangle_for_painting()->zrect2
  //     // only used by controller and paint()
  //   private absolute_rectangle_for_painting(zrect2)
  //     // only used by controller and adjust_to_fit
  //   private list_hook()->window * // only used by controller
  //   private list_hook(window *) // only used by controller
  //
  //   paint_traits()->const window_paint_traits &
  //   paint_traits(const window_paint_traits &)
  //
  //   has_focus()->bool
  //   protected focus(bool)
  //   protected virtual set_focus() // used by controller
  //     // do not call keys().set_focus()
  //   protected virtual unset_focus() // used by controller
  //     // do not call keys().unset_focus()
  //   // override the following functions to control inner key input
  //   // do not touch keys(), because keys() is controlled by controller
  //   //   set_focus()
  //   //   unset_focus()
  //   //   press()
  //   //   release() // do not call keys().release()
  //   //   rotate_wheel(int)
  //   //
  //   // for customized key input, use the following:
  //   //   window::has_keys()
  //   //   window::keys()
  //   //   window::set_keys()
  //   //   window::unset_keys()
  //
  //   virtual neutralize()
  //     // Call before extracting from the running window_tree,
  //     // then it's valid to just insert the running window_tree again.
  //     //
  //     // Focus affairs are left to controller.
  //     // Before insertion or erasion of the runging window_tree,
  //     // use the controller to unset focus.
  //
  //   virtual aware_keys()->bool
  //     // can override
  //     // ill formed if has_keys() && !aware_keys()
  //     // the only way to override: always return true
  //   has_keys()->bool
  //   set_keys()
  //   unset_keys()
  //   keys()->key_mapper &
  //   protected virtual press(key_code_t) // used by controller
  //     // focus change is left to controller
  //     // press trigger focus seting by default
  //     // window::press() include focus affair
  //     // inner press affairs are left to inheriented class
  //   protected virtual release(key_code_t) // used by controller
  //     // focus change is left to controller
  //     // release does not trigger focus change by default
  //     // window::release() do not affect focus affair
  //     // inner release affairs are left to inheriented class
  //   protected virtual rotate_wheel(int) // used by controller
  //     // rotate_wheel does not trigger focus change by default
  //
  //   position()->zvec2
  //   position(zvec2) // non-recursive, no extra operation
  //   move_to(zvec2) // with adjust_to_fit()
  //   min_size()->zvec2
  //   max_size()->zvec2
  //   protected min_size(zvec2) // non-recursive
  //   protected max_size(zvec2) // non-recursive
  //   protected lock_size() // assign size to minmax_size
  //   protected unlock_size() // assign {0, szt_numlim} to minmax_size
  //   size()->zvec2
  //   size(zvec2) // non-recursive, no extra operation
  //   protected virtual adjust_children_no_shrink() = 0 // non-recursive
  //   protected virtual adjust_to_fit_parent_no_shrink() = 0 // non-recursive
  //   protected virtual adjust_to_fit_children() = 0 // non-recursive
  //   protected adjust_to_fit() // recursive
  //   virtual resize(zvec2) // recursive
  //
  //   rectangle()->zrect2
  //   absolute_rectangle()->zrect2
  //     // may be zero size because limits of parents
  //   absolute_position()->zvec2 // return absolute_rectangle().left_top()
  //   absolute_size()->zvec2 // return absolute_rectangle().size()
  //   protected absolute_rectangle(zrect2)
  //     // only called by adjust_to_fit to update the stored data
  //
  //   hidden()->bool
  //   protected hidden(bool) // non-recursive
  //   virtual hide() // recursive
  //   virtual show() // recursive // iter().parent().hidden() == false
  //
  //   locked()->bool
  //   protected locked(bool) // non-recursive
  //   virtual lock() // recursive
  //   virtual unlock() // recursive // iter().parent().locked() == false
  //
  //   no_cross()->bool // note: non-customized_children all need to lock cross
  //   protected lock_cross()
  //   protected unlock_cross()
  //
  //   virtual customized_children_lim()->size_t // return max size_t by default
  //   virtual customized_children_begin()->window_const_iterator
  //     // [begin, xxx_begin()) is for inner children.
  //     // [xxx_begin() + xxx_lim(), end) is for customized children.
  //     // Inner children pop up to begin position at most.
  //     // Customized children pop up to be xxx_begin() at most.
  //     // The returned value should be used immediately to avoid invalidation
  //     // by insertion and erasion.
  //
  //   virtual no_overlap()->bool
  //
  //   protected virtual paint(pixmap &,
  //                           exclusive_thread_pool &,
  //                           bool no_overlap) = 0
  //
  // emplace_window<WINDOW_T>(window_tree &,
  //                          window_const_iterator,
  //                          s...)->WINDOW_T *
  // erase_window(window_tree &, WINDOW_T *)
}
void doc_root_window() {
  // root_window : public window
  //   // if a window tree is not empty, its root node must be root_window
  //   smf: only destructible
  //   root_window(window_iterator)
  //   root_window(window_iterator, zvec2 sz)
  //   virtual aware_keys() override->bool // return false
  //   protected size(zvec2)
  //   protected virtual adjust_children_no_shrink() override
  //   protected virtual adjust_to_fit_parent_no_shrink() override
  //   protected virtual adjust_to_fit_children() override
  //   virtual resize(zvec2) override
  //   protected virtual paint(pixmap &,
  //                           exclusive_thread_pool &,
  //                           bool no_overlap) override
  //
  // make_window_tree()->window_tree
  // make_window_tree(zvec2)->window_tree
  //
...
void doc_window_controller() {
  ...
  //   cursor_position()->zvec2
  //
  //   wins()->window_tree & // wins() is empty if not changed by user code
  //   focused_window()->window * // may be nullptr
  //   keyboard_focused_window()->window * // may be nullptr
  //
  //   set_focus()
  //   set_focus(window *)
  //     // call when set_focus() has been called but unset_focus() not
  //   unset_focus()
  //   press(key_code_t)
  //     // call when set_focus() has been called but unset_focus() not
  //   release(key_code_t)
  ...
  //   mouse_move(zvec2)
  //
  //   neutralize() // clear focus for following tree node operations
  //     // note: focus and keys are already controlled by controller
}
void doc_main_window_and_window_program() {
  // main_window
  //   smf: only default constructible
  //
  //   static handle()->HWND
  ...
  //   static cursor_position()->zvec2
  //   static wins()->window_tree & // wins() must have a root node
  //   static controller()->window_controller &
  //
  //   static d2d_paint(const pixmap &)
  ...
  // as_window_program<T>
  //
  // run_window_program<user_defined_window_program>(console_on = false)->int
  //
  // // user_defined_window_program
  // //   : as_window_program<user_defined_window_program>
  // //   create() // optional
  // //   paint() // optional
  // //   destroy() // optional
  // //   set_focus() // optional
  // //   unset_focus() // optional
  // //   press(key_code_t) // optional
  // //   release(key_code_t) // optional
  // //   rotate_wheel(int) // optional
  // //   mouse_move(size_t, size_t) // optional
  // //   resize(zvec2) // optional
}
void doc_colored_window() {
  // colored_window
  // can_be_top_window<colored_window> : true_type
  //
  // colored_window : public window
  //   smf: only destructible
  //   explicit colored_window(window_iterator) // set color to black
  //   colored_window(window_iterator, uint32_t color)
  //   colored_window(window_iterator, zvec2 sz) // set color to black
  //   colored_window(window_iterator, zvec2 sz, uint32_t color)
  //   color()->uint32_t
  //   color(uint32_t)
  //   lock_size(zvec2)
  //   unlock_size(zvec2)
  //   protected virtual void adjust_children_no_shrink() override;
  //   protected virtual void adjust_to_fit_parent_no_shrink() override;
  //   protected virtual void adjust_to_fit_children() override;
  //   protected virtual void paint(pixmap &,
  //                                exclusive_thread_pool &,
  //                                bool no_overlap) override
}
void doc_edge_hook() {
  // enum class zrect2_edge
  //   left
  //   top
  //   right
  //   bottom
  ...
  //   smf: only destructible
  //   explicit edge_hook(window_iterator)
  //   edge_hook(window_iterator, zrect2_edge)
  //   edge_hook(window_iterator, zrect2_edge, double k1, double k2)
  //     // for k1 and k2, the stored value is clamp(k, 0.0, 1.0)
}
...
int main() {
  ...
  re::inner::fns::doc_matrix();
  re::inner::fns::doc_dup_compressed_array();
  ...
  re::inner::fns::doc_shared_mutex();
  re::inner::fns::doc_thread_pool();
  re::inner::fns::doc_exclusive_thread_pool();

  re::inner::fns::doc_key_code();
  re::inner::fns::doc_key_combination();
  re::inner::fns::doc_key_recorder();
  re::inner::fns::doc_key_mapper();
  re::inner::fns::doc_vector_math();
  re::inner::fns::doc_rgba();
  re::inner::fns::doc_zrect2();
  re::inner::fns::doc_pixmap();
  re::inner::fns::doc_freetype();
  re::inner::fns::doc_window_paint_traits();
  re::inner::fns::doc_window();
  re::inner::fns::doc_root_window();
  re::inner::fns::doc_window_controller();
  re::inner::fns::doc_main_window_and_window_program();
  re::inner::fns::doc_colored_window();
  re::inner::fns::doc_edge_hook();
