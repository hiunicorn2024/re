...
void test_tree_iterator() {
  ...
  static_assert(can_apply_minus
                <const t::iterator &, const t::const_iterator &>);
  static_assert(can_apply_minus
                <const t::const_iterator &, const t::iterator &>);

  t x = t(1, t(2), t(3), t(4), t(5, t(6), t(7)));
  test_ritr(deref_rng(*x.root()), seq(2, 3, 4, 5));
  test_ritr(deref_rng(*x.croot()), seq(2, 3, 4, 5));
...
void test_tree_special_member_functions() {
  // with stateless allocator
  {
    ...
    pair<init_ft, eq_ft> a[] = {
      ...
      },
      {
        ...
        eq_ft([](const t *p)->bool {
          return !p->empty() && inner::good(*p)
            && (p->root() != typename t::iterator{} && **p->root() == 1)
            && (p->root()->size() == 3
  ...
  // with stateful allocator
  {
    ...
    using t = tree<int, alloc_t>;
    using init_ft = ez_function<void (t *)>;
    using eq_ft = ez_function<bool (const t *)>;
    const auto empt_f = [](const t *p)->bool {
      return p->empty() && p->root() == t::iterator{};
    };
...
void test_tree_node_operations() {
  ...
  // copy(i, get_key)
  {
    ...
    t x4 = x.copy(x.root()->begin()->begin() + 1,
    ...
    assert(x == mk(1,
                   ...
                   mk(3),
                   mk(4)));
    assert(x.copy(x.root()->cbegin()->cbegin() + 1, deref)
           == mk(0, mk(0), mk(0)));
...
void test_tree_mscl_for_sequence_container() {
  ...
  {
    t x(1, t(2), t(3));
    assert(x.capacity(x.root()) >= 2);
    x.reserve(x.root(), 100);
    assert(x.capacity(x.root()) >= 100);
    x = t(1, t(2, t(3), t(4)), t(5));
    x.pre_order_reserve(x.root(), 1000);
    for (auto &u : x.root().pre_order())
      assert(x.capacity(u.iter()) >= 1000);
  }
  ...
  // pre_order_shrink_to_fit(i)
  {
    ...
    x.pre_order_shrink_to_fit(x.root());
    for (auto &u : x.root().pre_order())
      assert(x.capacity(u.iter()) == u.size());
  }
...
void test_tree_vector_special_member_functions() {
  ...
  // with stateful allocator
  {
    using alloc_t = stateful_test_allocator<int, ez_map, false, false, false>;
    using t = tree<int, alloc_t>;
    using vec_t = tree_vector<int, alloc_t>;
    using init_ft = ez_function<void (vec_t *)>;
    using eq_ft = ez_function<bool (const vec_t *)>;
    const auto empt_f = [](const vec_t *p)->bool {
      return p->empty() && p->size() == 0 && inner::good(*p);
...
void test_tree_vector_construct_assign() {
  ...
  using vec_t = tree_vector<int, alloc_t>;
  using tt = tree<long, alloc_tt>;
  using vec_tt = tree_vector<long, alloc_tt>;
...
void test_tree_vector_mscl_for_sequence_container() {
  ...
  {
    vec_t v({1, 2});
    assert(v.capacity() == v.capacity(vec_t::iterator{}));
    assert(v.capacity(v.begin()) == v.capacity(next(v.begin())));
  }
  ...
  // pre_order_reserve(i, n);
  {
    ...
    v.reserve(vec_t::iterator{}, 100u);
    assert(v.capacity() >= 100u);
    v.pre_order_reserve(200u);
    assert(all_of(v.begin().pre_order(),
                  [&](auto &x) {return v.capacity(x.iter()) >= 200u;}));
  }
  ...
  // pre_order_shrink_to_fit(i)
  {
    vec_t v(t(1), t(2, t(3)));
    v.shrink_to_fit();
    v.shrink_to_fit(v.begin());
    v.pre_order_shrink_to_fit();
    v.pre_order_shrink_to_fit(v.begin());
  }
...
void test_tree() {
  ...
  inner::test::test_tree_base();
  inner::test::test_tree_iterator();
  inner::test::test_tree_special_member_functions();
  inner::test::test_tree_construct_assign();
  inner::test::test_tree_node_operations();
  ...
  inner::test::test_tree_vector_special_member_functions();
  inner::test::test_tree_vector_construct_assign();
  inner::test::test_tree_vector_node_operations();
  inner::test::test_tree_vector_insert_erase();
  inner::test::test_tree_vector_mscl_for_sequence_container();
