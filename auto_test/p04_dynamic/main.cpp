...
void test_dynamic_1() {
  test_dynamic_1_impl<as_dynamic, inner::empty_type>();
  test_dynamic_1_impl<inner::empty_type, as_dynamic>();
}
...
void test_dynamic_4() {
  test_dynamic_4_impl<as_dynamic>();
  test_dynamic_4_impl<inner::empty_type>();
}
void test_dynamic_cast_between_different_buffers() {
  struct A : private as_dynamic {
    virtual bool f() const noexcept {
      return false;
    }
  };
  struct B : A {
    int64_t a[4] = {1, 20, 300, 4000};
    virtual bool f() const noexcept {
      return a[0] + a[1] + a[2] + a[3] == 4321;
    }
  };
  struct C : A {
  ...
  using d_t0 = dynamic<A>;
  using d_t = dynamic<A, sizeof(B), 8>;
  using dm_t = dynamic<A, sizeof(B) - 8u, 8>;
  using dp_t = dynamic<A, sizeof(B) + 8u, 8>;
  d_t d;
  assert(d.empty());
  d = B{};
  assert(!d.empty() && d.local() && d->f());

  // construct by const dynamic<...> &
  {
    dm_t dm = d;
    assert(!d.empty() && d.local() && d->f());
    assert(!dm.empty() && !dm.local() && dm->f());
    dm_t dm2 = dm;
    assert(!dm.empty() && !dm.local() && dm->f());
    assert(!dm2.empty() && !dm2.local() && dm2->f());
    d_t0 dm0 = dm2;
    assert(!dm2.empty() && !dm2.local() && dm2->f());
    assert(!dm0.empty() && dm0->f());
    dm_t dm3 = dm0;
    assert(!dm0.empty() && dm0->f());
    assert(!dm3.empty() && !dm3.local() && dm3->f());
    dm_t dm4 = dm3;
    assert(!dm3.empty() && !dm3.local() && dm3->f());
    assert(!dm4.empty() && !dm4.local() && dm4->f());

    dp_t dp = d;
    assert(!d.empty() && d.local() && d->f());
    assert(!dp.empty() && dp.local() && dp->f());
    dp_t dp2 = dp;
    assert(!dp.empty() && dp.local() && dp->f());
    assert(!dp2.empty() && dp2.local() && dp2->f());
    d_t0 dp0 = dp2;
    assert(!dp2.empty() && dp2.local() && dp2->f());
    assert(!dp0.empty() && dp0->f());
    dp_t dp3 = dp0;
    assert(!dp0.empty() && dp0->f());
    assert(!dp3.empty() && dp3.local() && dp3->f());
    dp_t dp4 = dp3;
    assert(!dp3.empty() && dp3.local() && dp3->f());
    assert(!dp4.empty() && dp4.local() && dp4->f());
  }
  // assign const dynamic<...> &
  {
    dm_t dm = d;
    assert(!d.empty() && d.local() && d->f());
    assert(!dm.empty() && !dm.local() && dm->f());
...
void test_dynamic() {
  ...
  inner::test::test_dynamic_t();
  inner::test::test_dynamic_2();
  inner::test::test_dynamic_3();
  inner::test::test_dynamic_4();
  inner::test::test_dynamic_cast_between_different_buffers();
  inner::test::test_dynamic_void();
  inner::test::test_function();
  inner::test::test_unique_function();
  inner::test::test_move_only_function();
  inner::test::test_type_erased_invocation();
  inner::test::test_variant();
  inner::test::test_variant_visit();
