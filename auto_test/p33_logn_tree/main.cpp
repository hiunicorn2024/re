...
void test_dynamic_tree() {
  ...
  {
    dynamic_tree<key_t> t(in_place_type<key_t>);
    assert(!t.empty());
    assert(t.root()->value == 0);
  }
  {
    linked_dynamic_tree<key_t> t(in_place_type<key_t>);
    assert(!t.empty());
    assert(t.root()->value == 0);
  }
  {
    logn_dynamic_tree<key_t> t(in_place_type<key_t>);
    assert(!t.empty());
    assert(t.root()->value == 0);
  }
}
