...
void test_ranked_rbtree_adaptor() {
  using tree_t = ranked_rbtree_adaptor
    <ranked_rbtree_traits<ranked_rbtree_node<int>, 0, 1,
      ...
      for (int n2 : irng(0, 50)) {
        type_pack_for_each<P>
          ([n1, n2, init, good](auto a, auto b, auto c, auto d, auto e) {
            ...
            using t = ranked_rbtree_adaptor
              <ranked_rbtree_traits
               <ranked_rbtree_node<int>, 0, 1, alloc_t>>;
            alloc_t a1;
  ...
  // nth
  {
    ...
    static_assert(bidirectional_iterator<tree_t::iterator>);
    static_assert(bidirectional_iterator<tree_t::const_iterator>);
    static_assert(regular<tree_t::iterator>);
    static_assert(regular<tree_t::const_iterator>);

    {
      tree_t t;
      assert(t.nth(0) == t.end());
      assert(as_const(t).nth(0) == t.end());
      assert(t.end().nth() == 0);
      assert(t.nth(t.end()) == 0);
    }
    {
      tree_t t;
      copy(irng(0, 100), to_back(t));
      test_ritr(t.begin(), t.end(), irng(0, 100));
      test_ritr(t.begin(), t.cend(), irng(0, 100));
      test_ritr(t.cbegin(), t.end(), irng(0, 100));
      test_ritr(t.cbegin(), t.cend(), irng(0, 100));
      for (int i : irng(0, 100)) {
        const auto it = next(t.begin(), i);
        assert(t.nth(i) == it);
        assert(as_const(t).nth(i) == it);
        assert(it.nth() == i);
        assert(t.nth(it) == i);
      }
    }
  ...
  {
    tree_t t;
    copy(irng(0, 5), to_back(t));
    for (auto it = t.begin(); it != t.end();) {
      auto tmp = t.new_node(*it);
      const auto next_it = next(it);
      tmp = t.exchange_node(it, tmp);
      t.delete_node(tmp);
      it = next_it;
    }
    assert(inner::good(t));
    assert(equal(t, irng(0, 5)));
  }
  ...
  {
    tree_t t;
    assert(t.emplace_front(2) == 2);
    t.push_front(1);
    assert(*t.link_front(t.new_node(0)) == 0);
    assert(t.front() == 0);
    assert(equal(t, irng(0, 3)));
  ...
  {
    tree_t t;
    assert(t.emplace_back(0) == 0);
    t.push_back(1);
    assert(*t.link_back(t.new_node(2)) == 2);
    assert(t.back() == 2);
    assert(equal(t, irng(0, 3)));

    t.pop_back();
    assert(equal(t, irng(0, 2)));
    t.pop_back(2);
    assert(t.empty());
    t.pop_back(0);
    assert(t.empty());
  ...
  {
    tree_t t;
    copy(irng(0, 10), to_back(t));
    for (int i : irng(0, 10)) {
      assert(t[i] == i);
      assert(as_const(t)[i] == i);
      assert(t.at(i) == i);
      assert(as_const(t).at(i) == i);
    }
#ifndef RE_NOEXCEPT
    assert(test_throwing<out_of_range>([&]() {t.at(10);}));
    assert(test_throwing<out_of_range>([&]() {as_const(t).at(10);}));
#endif
  }
  ...
  // insert(cit, n, x)
  {
    ...
    {
      tree_t t(3);
      assert(equal(t, rng(3, 0)));
    }
    {
      tree_t t(3, a);
      assert(equal(t, rng(3, 0)) && t.get_allocator() == a);
    }
    {
      tree_t t(3, 1, a);
      assert(equal(t, rng(3, 1)) && t.get_allocator() == a);
    }
    {
      tree_t t;
      t.insert(t.end(), 3, 1);
      assert(equal(t, rng(3, 1)));
      t.assign(5, 10);
      assert(equal(t, rng(5, 10)));
    }
  }
  ...
  // insert(cit, il)
  {
    ...
    {
      tree_t t({1, 2, 3});
      assert(equal(t, irng(1, 4)));
    }
    {
      tree_t t({1, 2, 3}, a);
      assert(equal(t, irng(1, 4)) && t.get_allocator() == a);
    }
    {
      tree_t t;
  ...
  // assign(r)
  {
    stateful_test_allocator<int> a;
    {
      tree_t t(from_range, irng(1, 4));
      assert(equal(t, irng(1, 4)));
    }
    {
      tree_t t(from_range, irng(1, 4), a);
      assert(equal(t, irng(1, 4)) && t.get_allocator() == a);
    }
    {
      tree_t t(irng(1, 4));
    ...
    {
      tree_t t;
      t.insert(t.end(), irng(1, 4));
      assert(equal(t, irng(1, 4)));
      t.assign(irng(5, 7));
      assert(equal(t, irng(5, 7)));
      t = irng(1, 4);
      assert(equal(t, irng(1, 4)));
    }
  }
  ...
  {
    tree_t t;
    t.append_range(irng(1, 4));
    assert(equal(t, irng(1, 4)));
    t.push_back(irng(4, 7));
    assert(equal(t, irng(1, 7)));
    assert(equal(t.append(7).append(irng(8, 10)), irng(1, 10)));
  }
  ...
  {
    tree_t t;
    t.prepend_range(irng(1, 4));
    assert(equal(t, irng(1, 4)));
    t.push_front(irng(-2, 1));
    assert(equal(t, irng(-2, 4)));
    assert(equal(t.prepend(-3).prepend(irng(-7, -3)), irng(-7, 4)));
  }
  ...
  // resize(n, x)
  {
    ...
    assert(equal(t, rng(3, 0)));
    t.resize(1);
    assert(equal(t, rng(1, 0)));
    t.resize(3, 1);
    assert(equal(t, seq(0, 1, 1)));
    t.resize(0, 1);
    assert(t.empty());
  }

  // splice(next, this_ref)
  // splice(next, this_ref, cit)
  // splice(next, this_ref, cit, cit)
  {
    {
      tree_t a = {1, 2, 3};
      tree_t b({0, 0}, a.get_allocator());
      a.splice(a.nth(1), b);
      assert(equal(a, seq(1, 0, 0, 2, 3)));
      assert(b.empty());
      b.splice(b.end(), move(a));
      assert(a.empty());
      assert(equal(b, seq(1, 0, 0, 2, 3)));
    }
    {
      tree_t a = {1, 2};
      a.splice(a.begin(), a, prev(a.end()));
      assert(equal(a, seq(2, 1)));
      a.splice(a.begin(), move(a), prev(a.end()));
      assert(equal(a, seq(1, 2)));
    }
    {
      tree_t a = {1, 2, 3};
      a.splice(a.begin(), a, next(a.begin()), a.end());
      assert(equal(a, seq(2, 3, 1)));
      a.splice(a.begin(), move(a), next(a.begin()), a.end());
  ...
  {
    {
      ez_vector<int> v;
      tree_t t;
      copy(irng(0, 10), to_back(v));
      copy(irng(0, 10), to_back(t));
      for (int i : irng(0, 10))
        for (int j : irng(0, 10)) {
          if (i != j) {
            t.swap(t.nth(i), t.nth(j));
            adl_swap(ref(v, i), ref(v, j));
            assert(inner::good(t));
            assert(equal(t, v));

            t.swap(t.nth(i), t, t.nth(j));
            adl_swap(ref(v, i), ref(v, j));
            assert(inner::good(t));
            assert(equal(t, v));
          }
    ...
    }
    {
      for (int i : irng(0, 3))
        for (int j : irng(0, 3)) {
          tree_t t1 = {1, 2, 3};
          tree_t t2({4, 5, 6}, t1.get_allocator());
          t1.swap(nth(t1, i), t2, nth(t2, j));
          adl_swap(ref(t1, i), ref(t2, j));
          assert(t1.leftmost() == nth(t1, 0));
          assert(t1.rightmost() == nth(t1, 2));
          assert(t1.root() == nth(t1, 1));
          assert(t2.leftmost() == nth(t2, 0));
          assert(t2.rightmost() == nth(t2, 2));
          assert(t2.root() == nth(t2, 1));
          assert(equal(t1, irng(1, 4)));
          assert(equal(t2, irng(4, 7)));
        }
    }
  }
  ...
  // remove(x)
  {
    tree_t t = {0, 1, 4, 2, 2, 3, 4, 5};
    const auto it = next(t.begin());
    assert(t.erase_or_unlink(t.begin()) == it);
    assert(equal(t, seq(1, 4, 2, 2, 3, 4, 5)));

    assert(t.remove_if(bind(equal_to{}, _1, 2)) == 2);
    assert(equal(t, seq(1, 4, 3, 4, 5)));
    assert(t.remove(4) == 2);
    assert(equal(t, seq(1, 3, 5)));
  }
  ...
  // unique()
  {
    tree_t t = {1, 2, 2, 3, 4};
    t.unique();
    assert(equal(t, seq(1, 2, 3, 4)));
    t.unique([](int x, int y) {return x + 1 == y;});
    assert(equal(t, seq(1, 3)));
  }
  ...
  // merge(this_ref, less = ...)
  {
    {
      tree_t t = {1, 3, 5};
      tree_t t2({2, 4, 6}, t.get_allocator());
      t.merge(t2);
      assert(equal(t, irng(1, 7)));
      assert(t2.empty());
    }
  ...
  // reverse
  {
    ...
    t.append(irng(1, 4));
    t.reverse();
    assert(equal(t, rrng(irng(1, 4))));
    assert(*t.leftmost() == 3);
    assert(*t.rightmost() == 1);
    assert(*t.root() == 2);
  }
  ...
  // sort
  {
    ...
    assert(t.empty());
    t.append(irng(0, 100));
    assert(equal(t, irng(0, 100)));

    shuffle(rng(t.begin().promote(), t.end().promote()), minstd_rand{});
    assert(!equal(t, irng(0, 100)));
    t.sort();
    assert(equal(t, irng(0, 100)));
    ...
    assert(!equal(t, irng(0, 100)));
    t.sort(greater{});
    assert(equal(t, rrng(irng(0, 100))));
  }
...
void test_ranked_set_common_operations_impl(F mk, F2 key) {
  // special member functions
  {
...
void test_ranked_set_common_operations() {
  test_ranked_set_common_operations_impl
    <ranked_set<int, less<int>, stateful_test_allocator<int>>>
    (copy, identity{});
  test_ranked_set_common_operations_impl
    <ranked_map<int, int, less<int>,
                stateful_test_allocator<pair<int, int>>>>
    (bind(make_pair, _1, _1), at<0>);
}

template <class S, class F, class F2>
void test_ranked_multiset_common_operations_impl(F mk, F2 key) {
  // special member functions
  {
    static_assert(regular<S>);
    static_assert(is_nothrow_movable_v<S>);
...
void test_ranked_multiset_common_operations() {
  test_ranked_multiset_common_operations_impl
    <ranked_multiset<int, less<int>, stateful_test_allocator<int>>>
    (copy, identity{});
  test_ranked_multiset_common_operations_impl
    <ranked_multimap<int, int, less<int>,
                     stateful_test_allocator<pair<int, int>>>>
    (bind(make_pair, _1, _1), at<0>);
}

template <class S>
void test_ranked_map_specialized_operations_impl() {
...
void test_ranked_map_specialized_operations() {
  test_ranked_map_specialized_operations_impl
    <ranked_map<int, int, less<int>, test_allocator<pair<int, int>>>>();
  test_ranked_map_specialized_operations_impl
    <ranked_map<int, int, less<>, test_allocator<pair<int, int>>>>();
}

void test_ranked_map() {
  printf("ranked_map: ");

  inner::test::test_ranked_rbtree_adaptor();
  inner::test::test_ranked_set_common_operations();
  inner::test::test_ranked_multiset_common_operations();
  inner::test::test_ranked_map_specialized_operations();
